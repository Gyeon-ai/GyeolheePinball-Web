import { UI_FONT_FAMILY, winnerAreaHeight } from './data/constants';
...
function drawPreroll(ctx: CanvasRenderingContext2D, w: number, h: number, ad: RoundAd, images: AdImages): void {
  ...
  ctx.font = `700 ${provideSize}px ${UI_FONT_FAMILY}`;
  ...
  ctx.font = `600 ${sponsorSize}px ${UI_FONT_FAMILY}`;
  ...
  ctx.font = `500 ${labelSize}px ${UI_FONT_FAMILY}`;
...
function drawResult(
  ...
  if (ad.tagline) {
    ctx.font = `400 ${taglineSize}px ${UI_FONT_FAMILY}`;
